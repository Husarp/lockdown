...
class ProtectionTab(ctk.CTkScrollableFrame):
    def __init__(self, master, page):
        ...
        ctk.CTkLabel(allowed.body, text="Sites a list blocks by mistake. They're opened again within a few seconds.",
                     text_color=MUTED, font=theme.body(11)).pack(anchor="w")
        ...
        self._lists_cache: dict[str, set] = {}
    ...
    def refresh(self, now=None, usage=None):
        ...
        for key, sw in self.switches.items():
            ...
            if info.get("count"):
                ...
                text = "Downloading... (the service does it within a minute; it needs to be running)"
            else:
                text = "Off"
    ...
    def _update_now(self):
        ...
        protection.save_settings(self.db, cfg)
        self.page.confirm("Lists will update within a minute", immediate=True)
    ...
    def _check(self):
        ...
        cfg = protection.settings(self.db)
        found = []
        for key, (name, _what, _url, _default) in protection.LISTS.items():
            if key not in self._lists_cache:
                self._lists_cache[key] = set(protection.load(key))
            if host in self._lists_cache[key] or f"www.{host}" in self._lists_cache[key]:
                found.append(name + ("" if key in cfg["enabled"] else " (list is off)"))
